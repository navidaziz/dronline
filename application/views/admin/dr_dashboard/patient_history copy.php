...
<div class="row">
  ...
  <div class="col-md-6">
      ...
      <div class="box-body">
        ...
        <page size='A4'>
          ...
          <div>
            ...
            <h4>Visits</h4>
            <?php
            $query = "SELECT * FROM patient_visits WHERE patient_id = '" . $invoice_detail->patient_id . "'";
            $visits = $this->db->query($query)->result();
            $visit_id = 0;
            foreach ($visits as $visit) { ?>
              <h4>Visit No. <?php echo $visit->visit_no; ?></h4>
              <div>
                <!-- <div class="col-md-6">
                ...
                <form onsubmit="return confirm('Do you really want to submit the form?');" action="<?php echo site_url(ADMIN_DIR . "dr_dashboard/add_visit"); ?>" method="post">
                  <input type="hidden" value="<?php echo $invoice_detail->patient_id; ?>" name="patient_id" />
                  <input type="hidden" value="<?php echo $invoice_detail->invoice_id; ?>" name="history_id" />
                  <input class="btn btn-success" type="submit" name="add new visit" value="add new visit" />
                </form>
              </div>
              <br />
            <?php }  ?>
    ...
    <form onsubmit="return confirm('Do you really want to submit the form?');" action="<?php echo site_url(ADMIN_DIR . "dr_dashboard/add_visit"); ?>" method="post">
      <input type="hidden" value="<?php echo $invoice_detail->patient_id; ?>" name="patient_id" />
      <input type="hidden" value="<?php echo $invoice_detail->invoice_id; ?>" name="history_id" />
      <input type="submit" name="add new visit" value="add new visit" />
    </form>
